...
func (c *Config) GetTarget() (targetiface.Target, error) {
	...
	case "rabbitmq":
		plug = target.AdaptRabbitMQTargetFunc(
			target.RabbitMQTargetConfigFunction,
		)
	default:
		return nil, errors.New(fmt.Sprintf("Invalid target found; expected one of 'stdout, kinesis, pubsub, sqs, kafka, eventhub, http, rabbitmq' and got '%s'", useTarget.Name))
	}
...
func (c *Config) GetFailureTarget(AppName string, AppVersion string) (failureiface.Failure, error) {
	...
	default:
		return nil, errors.New(fmt.Sprintf("Invalid failure target found; expected one of 'stdout, kinesis, pubsub, sqs, kafka, eventhub, http, rabbitmq' and got '%s'", useFailureTarget.Name))
	}
