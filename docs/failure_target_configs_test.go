...
// TestFailureTargetDocsFullConfigs tests config examples who have all options fully populated.
...
func TestFailureTargetDocsFullConfigs(t *testing.T) {
	assert := assert.New(t)
	...
	for _, trgt := range targetsToTest {
		hclFilename := filepath.Join("configs", "failure-targets", "full", fmt.Sprintf("%s-full.hcl", trgt))
...
// TestFailureTargetDocsMinimalConfigs tests minimal config examples.
func TestFailureTargetDocsMinimalConfigs(t *testing.T) {
	assert := assert.New(t)

	targetsToTest := []string{"eventhub", "http", "kafka", "kinesis", "pubsub", "sqs", "stdout"}

	for _, trgt := range targetsToTest {
		hclFilename := filepath.Join("configs", "failure-targets", "minimal", fmt.Sprintf("%s-minimal.hcl", trgt))
		t.Setenv("STREAM_REPLICATOR_CONFIG_FILE", hclFilename)
