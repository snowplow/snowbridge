...
func TestSpCollectorPayloadThriftToJSON(t *testing.T) {
	assert := assert.New(t)
	...
	// Simple success case
	transformSuccess, _, failure, _ := SpCollectorPayloadThriftToJSON(&messageGood, nil)
	...
	// Simple failure case
	success, _, transformFailure, _ := SpCollectorPayloadThriftToJSON(&messageBad, nil)
	...
	if transformFailure.GetError() != nil {
		assert.Equal("Unknown data type 110", transformFailure.GetError().Error())
	}
